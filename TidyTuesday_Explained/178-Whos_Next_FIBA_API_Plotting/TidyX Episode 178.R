### TidyX Episode 178 - Player Charts
...
t1_players_over_time_dummy_vars <- lineup_over_time_df %>% 
  ...
  pivot_wider(
    ...
    values_from = is_in,
    values_fn = unique, ## repeated values for the same row number, how to handle it
    values_fill = 0
  )

t1_players_over_time_dummy_vars_time <- t1_players_over_time_dummy_vars %>% 
  mutate(
    # Make quarter time
    elapsed_quarter_time = case_when(
      periodtype == "REGULAR" ~ 10 - as.numeric(format(as.POSIXct(gt, format = "%M:%OS"), "%M.%S")),
      periodtype == "OVERTIME" ~ 5 - as.numeric(format(as.POSIXct(gt, format = "%M:%OS"), "%M.%S"))
    ),
    
    elapsed_game_time = case_when(
      periodtype == "REGULAR" ~ elapsed_quarter_time +  ((period-1) * 10),
      periodtype == "OVERTIME" ~ elapsed_quarter_time + ((period-1) * 5) + 40
    )
  ) %>% 
  pivot_longer(
    ## Get all the player columns
    5:(ncol(.) -2),
    ## pivot the names into the "player" columns
    names_to = "player"
    ) %>% 
  group_by(player) %>% 
  ## Get the start of the game, and then from there on any time they went in or out
...
max_time <- 40
if( "OVERTIME" %in% pbp_df$periodType ){
  max_time <- max_time + 5*(max(unique(pbp_df$period[pbp_df$periodType == "OVERTIME"])))
}
  
t1_players_over_time_dummy_vars_time_maxtime <- t1_players_over_time_dummy_vars_time %>% 
  mutate(
    t2 = case_when(
      is.na(t2) ~ max_time, 
      TRUE ~ t2
...
## Player Plot
t1_players_over_time_dummy_vars_time_maxtime %>% 
  filter(
    value == 1,
  ) %>% 
  mutate(
